...
import { GroceryList, GroceryListItem, Ingredient, RecipeGroceryList, RecipeIngredient, RecipeList, User, UserIngredient } from './entities';
    ...
    await dataSource.transaction(async (transactionalEntityManager) => {
      ...
      const userIngredients = await Promise.all(
        Array(5).fill("").map(async () => {
          const userIngredient = await factoryManager.get(UserIngredient).make();
          userIngredient.user = faker.helpers.arrayElement(users); // Assign a random user
          return await transactionalEntityManager.save(UserIngredient, userIngredient);
        })
      );
      ...
      const groceryListItems = await Promise.all(
        Array(20).fill("").map(async () => {
          const groceryListItem = await factoryManager.get(GroceryListItem).make();
          groceryListItem.groceryList = faker.helpers.arrayElement(groceryLists); // Assign to a random list
          groceryListItem.ingredient = faker.helpers.arrayElement(ingredients); // Assign a random ingredient
